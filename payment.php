...
$status = FALSE;
if ( authorize($_SESSION["access"]["CHECKOUT"]["PAYMENT"]["create"]) || 
authorize($_SESSION["access"]["CHECKOUT"]["PAYMENT"]["edit"]) || 
authorize($_SESSION["access"]["CHECKOUT"]["PAYMENT"]["view"]) || 
authorize($_SESSION["access"]["CHECKOUT"]["PAYMENT"]["delete"]) ) {
 $status = TRUE;
}
...
<div class="row">
    <div class="col-lg-9">
        ...
        <?php if (authorize($_SESSION["access"]["CHECKOUT"]["PAYMENT"]["create"])) { ?>
                ...
                <tbody><tr>
                    ...
                    <?php for ($i = 1; $i <= 10; $i++) { ?>
                        <tr>
                            ...
                            <td>Sample content</td>
                            <td>
                                <?php if (authorize($_SESSION["access"]["CHECKOUT"]["PAYMENT"]["edit"])) { ?>
                                    <button class="btn btn-sm btn-info" type="button"><i class="fa fa-edit"></i> EDIT</button> 
                                <?php } ?>
                                <?php if (authorize($_SESSION["access"]["CHECKOUT"]["PAYMENT"]["view"])) { ?>
                                    <button class="btn btn-sm btn-warning" type="button"><i class="fa fa-search-plus"></i> VIEW</button>
                                <?php } ?>
                                <?php if (authorize($_SESSION["access"]["CHECKOUT"]["PAYMENT"]["delete"])) { ?>
